...
export default (props) => {
    ...
    const getPoints = (q) => {
        return new Promise(async (r, e) => {
            ...
            let url = `/api/v1/admin/user/manage/member/detail/sponsored/point?user_id=${match.params.id}`
    ...
    const getDataList = (q) => {
        return new Promise(async (r, e) => {
            ...
            let url = `/api/v1/admin/user/manage/member/detail/sponsored/list?user_id=${match.params.id}&limit=${limit}&offset=${offset}`
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            fullWidth
                            value={point ? numberWithCommas(point["total_sponsored"]) + " P" : ""}
                            InputProps={{
                                readOnly: true,
                    ...
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            InputLabelProps={{shrink: true}}
                            fullWidth
                            value={point ? numberWithCommas(point["total_sponsored_video"]) + " P" : ""}
                            InputProps={{
                                readOnly: true,
                                classes: {input: classes.paddingLT}
                            }}
                    ...
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            InputLabelProps={{shrink: true}}
                            fullWidth
                            value={point ? numberWithCommas(point["total_sponsored_creator"]) + " P" : ""}
                            InputProps={{
                                readOnly: true,
                                classes: {input: classes.paddingLT}
                            }}
                        ...
                        <TableBody>
                            {state[0] ? state.map((row, index) => (
                                <TableRow key={row.id} hover>
                                    ...
                                    <TableCell className={classes.tableCell}
                                               align="center">
                                        {row.video_post_id > 0 ? `${row.sponsor_nickname}이 영상 ${row.title} 에 후원`
                                            : row.video_post_id === 0 ? `${row.sponsor_nickname}이 크리에이터에 후원` : ""}
                                    </TableCell>
                                    <TableCell className={classes.tableCell}
                                               align="center"
