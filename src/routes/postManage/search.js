...
export default (props) => {
    const { classes, history, match, style, modalOpen, modalHandler, addVideos, videos, deleteVideos, unchkVideos, initialSelect } = props;
    ...
    const dataReq = () => {
        return new Promise(async (r, e) => {
            ...
            let url = `/api/v1/admin/post/manage/list/no-blind?limit=${limit}&offset=${offset}&keyword=${search}`;
            const headers = {
    ...
    const handleClick = (event, id, val) => {
        ...
        if (selectedIndex === 0) {
            ...
            newSelected = newSelected.concat(adding.slice(0, selectedIndex), adding.slice(selectedIndex + 1));

            newVideo = newVideo.concat(adding.slice(0, selectedIndex), adding.slice(selectedIndex + 1));
        }
    ...
    return (
        <>
            <DialogAlert open={open[0]} handleClose={() => setOpen({ ...open, 0: false })} text={'추가 하시겠습니까?'} fn={() => addVideo()} />
                        ...
                        <TableContainer>
                            <Table className={classes.table}>
                                ...
                                <TableBody>
                                    {state[0] ? (
                                        state.map((row, index) => (
                                            <TableRow id={'t' + row.id} key={row.id} hover style={isSelected(row.id) ? { backgroundColor: '#E0E7F7' } : { backgroundColor: 'white' }}>
                                                <TableCell padding="checkbox">
                                                    <YellowCheckbox
                                                        id={'c' + row.id}
