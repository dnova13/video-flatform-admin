...
export default (props) => {
    ...
    const moreBtnHandler = (d) => {
        setSession()
        history.push('/admin/service/intro/item/{0}'.format(d.id))
    }
    ...
    const dataReq = () => {
        return new Promise(async (r, e) => {
            ...
            let url = `/api/v1/admin/service/intro/list?limit=${limit}&offset=${offset}&keyword=${search}`
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} sm={2}>
                        <Button
                            ...
                            onClick={() => {
                                setSession()
                                history.push('/admin/service/intro/item')
                            }}
                        >추가</Button>
                    </Grid>
                    <Grid item xs={12} sm={2}>
                        ...
                        <Table className={classes.table}>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell className={classes.tableCell} align="center">제목</TableCell>
                                    <TableCell className={classes.tableCell} align="center">파일명</TableCell>
                                    <TableCell className={classes.tableCell} align="center">등록자</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {state[0] ? state.map((row, index) => (
                                    <TableRow key={row.id} hover>
                                        ...
                                        <TableCell className={classes.tableCell}
                                            ...
                                            <Link href={row.file ? process.env.REACT_APP_API_URL + row.file : ""} >{row.name}</Link>
                                        </TableCell>
                                        <TableCell className={classes.tableCell} align="center"
                                            onClick={() => moreBtnHandler(row)}>{"관리자"}</TableCell>
                ...
                <Grid item xs={12}>
                    <Pagination count={total} page={page} size="large" variant="outlined" shape="rounded"
                        ...
                        onChange={(e, v) => {
                            ...
                            setPage(v)
                            setSession(v)
                            // history.push(`/admin/service/intro/list?p=${v}&s=${search}`)
                        }} />
                </Grid>
            </Grid>
        </>
